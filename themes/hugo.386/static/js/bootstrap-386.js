(function () {
    'use strict';

    var character = { height: 20, width: 12.4 };

    function scrollLock() {
        var last = 0;
        window.addEventListener('scroll', function () {
            var off = window.scrollY;
            var func = off < last ? 'floor' : 'ceil';
            if (off % character.height === 0) { last = off; return; }
            last = off;
            window.scrollTo(0, Math[func](off / character.height) * character.height);
        });
    }

    function loading() {
        if (_386.fastLoad) {
            document.body.style.visibility = 'visible';
            return;
        }

        var onePass = _386.onePass;
        var speedFactor = 1 / (_386.speedFactor || 1) * 165000;
        var wrap = document.createElement('div');
        var bar = wrap.appendChild(document.createElement('div'));
        var cursor = document.createElement('div');
        var pass = (window.getComputedStyle(document.body).visibility === 'visible') ? 1 : 0;
        var height = window.innerHeight;
        var width = window.innerWidth;
        var rounds = (height * width / speedFactor);
        var column = width;
        var row = height - character.height;

        wrap.id = 'wrap386';
        bar.id = 'bar386';
        cursor.id = 'cursor386';
        cursor.innerHTML = bar.innerHTML = '&#9604;';

        if (pass === 0) {
            document.body.appendChild(wrap);
            document.body.style.visibility = 'visible';
        } else {
            document.body.appendChild(cursor);
            rounds /= 2;
            character.height *= 4;
        }

        var ival = setInterval(function () {
            for (var m = 0; m < rounds; m++) {
                column -= character.width;
                if (column <= 0) {
                    column = width;
                    row -= character.height;
                }
                if (row <= 0) {
                    pass++;
                    row = height - character.height;
                    if (pass === 2) {
                        document.body.removeChild(cursor);
                        clearInterval(ival);
                    } else {
                        wrap.parentNode.removeChild(wrap);
                        if (onePass) {
                            clearInterval(ival);
                        } else {
                            document.body.appendChild(cursor);
                            rounds /= 2;
                            character.height *= 4;
                        }
                    }
                }
                if (pass === 0) {
                    bar.style.width = column + 'px';
                    wrap.style.height = row + 'px';
                } else {
                    cursor.style.right = column + 'px';
                    cursor.style.bottom = row + 'px';
                }
            }
        }, 1);
    }

    // Scripts load after </body>, so DOMContentLoaded may have already fired.
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', loading);
    } else {
        loading();
    }
}());
